...
export default function Dashboard(props: { disableCustomTheme?: boolean }) {
	...
	const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
		...
		'access_mode' : access_mode
			...
			setLinkError(true);
			setLinkErrorMessage('Failed to edit link');
		});
	};
	...
	const fetchLinkDetails = async () => {
		...
		axios.get('https://linksy-mhe5.onrender.com/app/link?link_id=' + linkID, {
			...
			destination.value = link.referenced_link;
			title.value = link.title === undefined ? '' : link.title;
			category.value = link.access_mode === 0 ? 'option1' : link.access_mode === 1 ? 'option3' : 'option2';
			setCategory(category.value);
			if (link.access_mode === 2) {
				const blacklistedIps = link.black_listed_ips.map(ip => {
					return `${(ip >> 24) & 255}.${(ip >> 16) & 255}.${(ip >> 8) & 255}.${ip & 255}`;
				});
				blacklisted.value = blacklistedIps.join('\n');
			} else if (link.access_mode === 1) {
				const whitelistedIps = link.allowed_ips.map(ip => {
					return `${(ip >> 24) & 255}.${(ip >> 16) & 255}.${(ip >> 8) & 255}.${ip & 255}`;
				});
				whitelisted.value = whitelistedIps.join('\n');
			}
		}).catch(error => {
			console.log(error);
		});
